...
**마이페이지** 구현을 통해 회원이 자신의 회원 정보를 조회할 수 있고, 정보 수정이나 본인의 개성을 나타낼 수 있도록 프로필 사진을 변경할 수 있습니다.
...
그리고, **쪽지 페이지**에서는 자신이 대화를 나눈 회원들의 쪽지를 회원 별로 분류하여 날짜순으로 표시하였습니다. 회원은 다른 회원의 닉네임을 입력하여 그 회원에게 쪽지를 주고 받을 수 있으며 커뮤니티에서 게시글/댓글로 소통할 때는 한계가 있지만 이런 쪽지 기능을 통해 여러 회원과 더 다양하고 활발하게 소통할 수 있습니다.
...
**팀 모집 게시판**
이번에 구현한 팀 모집 게시판은 사용자가 팀원을 모집하고, 다른 사용자들이 해당 팀에 자유롭게 지원할 수 있도록 구성한 기능입니다. 사용자는 먼저 /board/team-recruit 경로로 접속하면, 기존에 작성된 팀 모집 게시글 목록을 확인할 수 있습니다. 게시글은 최신순으로 정렬되어 있으며, 상단에는 필터바가 적용되어 있어 팀명, 제목, 작성자 닉네임 기준으로 검색하거나, 현재 모집 중인 게시글만 따로 필터링하여 볼 수 있도록 구성했습니다. 많은 게시글이 등록된 경우에도 페이지네이션을 통해 쉽게 탐색할 수 있도록 구현했습니다.
목록 하단에 위치한 ‘게시글 작성’ 버튼을 클릭하면 /board/team-recruit/write 경로로 이동하며, 팀명, 제목, 팀 소개 내용을 입력하여 새로운 모집글을 작성할 수 있습니다. 이때 로그인된 사용자의 정보는 자동으로 반영되며, 게시글 작성 시 함께 저장됩니다. 작성된 글은 기본적으로 모집 중 상태로 등록되며, 지원자 정보를 담을 applicants 배열도 초기화된 상태로 포함되어 이후 지원 기능과 연동되도록 했습니다.
게시글을 클릭하면 상세 페이지(/board/team-recruit/:id)로 이동하며, 이곳에서는 글의 제목, 팀명, 작성자, 작성일, 상세 내용 등을 확인할 수 있습니다. 본인이 작성한 게시글인 경우에는 '삭제' 버튼과 함께 ‘지원자 확인’ 버튼도 나타납니다. 반대로 다른 사용자가 해당 상세 페이지에 들어왔을 경우, 우측 상단에 ‘신청하기’ 버튼이 활성화되며, 이를 통해 해당 팀에 지원할 수 있습니다. 단, 모집이 종료된 경우에는 이 버튼이 비활성화되며, “모집이 종료되었습니다”라는 안내 문구가 표시됩니다.
‘신청하기’ 버튼을 클릭하면 모달 창이 열리며, 사용자는 자기소개와 블로그 주소 등 간단한 정보를 입력해 지원할 수 있습니다. 이때 몇 가지 조건 검증을 거치게 되는데, 먼저 로그인하지 않은 사용자는 신청이 불가능하며, 본인이 작성한 글에는 지원할 수 없도록 제한했습니다. 또한, 이미 지원한 이력이 있는 경우에도 중복 지원이 불가능하도록 처리했습니다. 모든 조건을 만족할 경우, 사용자가 입력한 정보와 함께 사용자 닉네임, ID 등이 applicants 배열에 추가되며 게시글 데이터가 업데이트됩니다.
작성자가 ‘지원자 확인’ 버튼을 누르면 지원자 목록 페이지로 이동하며, 해당 글에 지원한 사용자들의 정보를 카드 형태로 확인할 수 있습니다. 각 카드는 지원자의 닉네임, 자기소개, 프로필 이미지, 수락/거절 버튼 등을 포함하고 있으며, 1행에 2개씩 정렬되어 깔끔한 레이아웃으로 구성했습니다. 만약 지원자가 아무도 없는 경우에는 “지원자가 없습니다”라는 문구가 중앙에 표시됩니다. 상단에는 모집 상태를 변경할 수 있는 버튼도 제공하며, 팀장은 이를 통해 ‘모집 종료’ 또는 ‘모집중’ 상태를 직접 전환할 수 있습니다. 만약 해당 글이 모집 종료 상태라면, 지원자 확인 페이지 접근 시 경고 메시지를 띄우고 자동으로 팀 모집 게시판 목록 페이지로 이동하도록 처리했습니다. 전체적인 흐름은 사용자가 처음 팀 모집 글을 탐색하고, 관심 있는 팀에 지원하거나, 반대로 팀장을 맡아 글을 등록하고 지원자를 관리하는 과정까지 자연스럽게 이어질 수 있도록 설계했습니다. 구조는 FilterBar, PostRow, Pagination, ApplyModal, ApplicantCard, ChangeRecruitmentStatusButton 등의 컴포넌트 단위로 모듈화하여 재사용성과 유지보수성을 높였으며, 사용자 경험을 고려한 예외 처리와 조건 분기 또한 꼼꼼히 구현했습니다.
...
|김성민|석현님은 항상 유쾌한 유머로 우리팀의 분위기를 밝게 해주고 그리고 일적인 부분에서도 자신의 파트를 빠르고 정확하게 해내는 모습이 너무 보기 좋았습니다. 석현님이 있어서 지난 백엔드 프로젝트 때부터 재밌게 지낼 수 있었습니다. 그리고 발표도 평소의 재치있고 유쾌한 모습 그대로 재밌고 우리 조원들이 열심히 노력이 잘 묻어나오게 잘해주셔서 감사합니다. |
|서민종|팀 내 분위기 메이커로서 많은 존재감을 가진 팀원이었습니다. 프로젝트 진행할 때의 분위기와 팀원간의 연결다리 역할 부분에서 최고로 기여를 많이 해주셨습니다. 또한, 카테고리별 신고 로직을 구성하여 회원 탈퇴까지 이어지는 서비스 흐름을 파악하고 잘 구현해 주셨습니다. 발표 전날에는 발표자료 준비를 위한 사전 더미데이터 세팅과 발표 흐름을 위한 전체 시나리오를 적절히 구성해주셔서 훌륭한 발표를 만들어낼 수 있었던 것 같습니다.|
...
|서민종|프론트엔드 프로젝트를 접하면서 처음에 많이 힘들어하신 것 같습니다. 하지만 포기하지 않고 계속 시도하면서 어떻게든 문제를 해결하려는 자세에 인상을 깊이 받았습니다. 팀 모집 게시판과 별점 리뷰 페이지를 구현하면서 복잡한 테이블 구조와 테이블 간의 로직을 구현하는 난이도가 어려운데도 훌륭히 구현해 주셨습니다. 또한, 서비스 구현의 과정에서도 끊임없이 전체 모델링 구조에 대해서 고민하고 개선하려는 노력을 많이 해 주신 덕분에 전체 모델 구조가 더 안정적으로 완성될 수 있었다고 생각합니다.|
